...
$ ->
	...
	$(document).keydown (event) ->
		body = window.player.GetBody()

		switch event.keyCode
			when 37 then body.ApplyForce(new Box2D.Common.Math.b2Vec2(-90, 0), body.GetWorldCenter())
			when 38 then body.ApplyForce(new Box2D.Common.Math.b2Vec2(0, -90), body.GetWorldCenter())
			when 39 then body.ApplyForce(new Box2D.Common.Math.b2Vec2(90, 0), body.GetWorldCenter())
			when 40 then body.ApplyForce(new Box2D.Common.Math.b2Vec2(0, 90), body.GetWorldCenter())
